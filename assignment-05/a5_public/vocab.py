...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        return [[[self.char2id['{']] + [self.char2id[c] for c in word] + [self.char2id['}']]
                 for word in s]
                for s in sents]
        ### END YOUR CODE
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        char_indices_sents = self.words2charindices(sents)
        padded_sents = pad_sents_char(char_indices_sents, self.char2id['<pad>'])
        sents_var = torch.tensor(padded_sents, dtype=torch.long, device=device).transpose(0, 1)
        return sents_var
        ### END YOUR CODE
